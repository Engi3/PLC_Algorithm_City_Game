...
import { evalContact, evalRungEnergized } from "@/lib/ladder/engine";
import type { Inputs, LadderProgram, Output, SimMemory } from "@/lib/ladder/types";
          ...
          <div key={rung.id} className="flex items-center gap-3 overflow-x-auto pb-1">
            ...
            <div className="flex flex-col gap-1">
              {rung.branches.map((branch, ri) => {
                const contacts = branch.cells.filter((c) => c !== null);
                return (
                    ...
                    {contacts.map((c, ci) => (
                      <Fragment key={ci}>
                        ...
                        <ContactBox
                          address={c!.address}
                          isNc={c!.type === "NC"}
                          energized={evalContact(c, inputs, memory)}
                        />
                      </Fragment>
                    ))}
                  </div>
